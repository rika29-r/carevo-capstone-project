...
const syncFormPath = (index) => {
  const path = stepPathMap[normalizeStepIndex(index)] || '/form/profile';
  if (window.location.pathname !== path) {
    window.history.pushState({ page: 'form', step: normalizeStepIndex(index) }, '', path);
  }
};
...
const mergeInitialFormData = (incoming = {}) => ({
  personalInfo: { ...initialFormData.personalInfo, ...(incoming.personalInfo || {}) },
  experience: { ...initialFormData.experience, ...(incoming.experience || incoming.experiences?.[0] || {}) },
  experiences: Array.isArray(incoming.experiences) ? incoming.experiences : [],
  education: { ...initialFormData.education, ...(incoming.education || incoming.educations?.[0] || {}) },
  educations: Array.isArray(incoming.educations) ? incoming.educations : [],
  skills: { ...initialFormData.skills, ...(incoming.skills || {}) },
  skillList: Array.isArray(incoming.skillList) ? incoming.skillList : [],
  projects: { ...initialFormData.projects, ...(Array.isArray(incoming.projects) ? incoming.projects[0] : incoming.projects || incoming.project || {}) },
  projectList: Array.isArray(incoming.projectList) ? incoming.projectList : (Array.isArray(incoming.projects) ? incoming.projects : []),
  certifications: { ...initialFormData.certifications, ...(Array.isArray(incoming.certifications) ? incoming.certifications[0] : incoming.certifications || incoming.certification || {}) },
  certificationList: Array.isArray(incoming.certificationList) ? incoming.certificationList : (Array.isArray(incoming.certifications) ? incoming.certifications : []),
  languages: { ...initialFormData.languages, ...(Array.isArray(incoming.languages) ? incoming.languages[0] : incoming.languages || incoming.language || {}) },
  languageList: Array.isArray(incoming.languageList) ? incoming.languageList : (Array.isArray(incoming.languages) ? incoming.languages : []),
});
...
function FormPage({
  ...
  const [activeStep, setActiveStep] = useState(() => normalizeStepIndex(initialStep));
  const [formData, setFormData] = useState(() => mergeInitialFormData(initialData || {}));
  ...
  useEffect(() => {
    setActiveStep(normalizeStepIndex(initialStep));
  }, [initialStep]);

  useEffect(() => {
    if (initialData) {
      setFormData(mergeInitialFormData(initialData));
      setProfileImage(initialData?.personalInfo?.profileImage || '');
    }
  }, [initialData]);

  useEffect(() => {
    syncFormPath(activeStep);
  }, [activeStep]);
  ...
  const validatePersonalInfo = () => {
    ...
    const careerInterest =
      data.careerInterest?.trim() || data.professionalTitle?.trim() || "";
    ...
    if (!careerInterest) {
      showNotice("error", "Data Belum Lengkap", "Career Interest wajib diisi.");
      return false;
    }
    ...
    setFormData((prev) => ({
      ...
      personalInfo: {
        ...prev.personalInfo,
        careerInterest,
        professionalTitle: careerInterest,
      },
    }));
  ...
  const validateExperience = () => {
    ...
    if (!data.employmentType?.trim()) {
      showNotice("error", "Data Belum Lengkap", "Employment Type wajib dipilih.");
      return false;
    }
  ...
  const validateSkills = () => {
    ...
    if (!data.skillCategory?.trim()) {
      showNotice("error", "Data Belum Lengkap", "Skill Category wajib dipilih.");
      return false;
    }

    if (!data.proficiencyLevel?.trim()) {
      showNotice("error", "Data Belum Lengkap", "Proficiency Level wajib dipilih.");
      return false;
    }
    ...
    if (selectedSkills.length === 0) {
      showNotice(
        "error",
        "Data Belum Lengkap",
        "Minimal pilih atau tambahkan 1 skill."
      );
      return false;
    }
  ...
  const buildCurrentDraft = () => {
    ...
    if (formData.experience?.jobTitle || formData.experience?.companyName) {
      const list = Array.isArray(formData.experiences) ? formData.experiences : [];
      const current = { ...formData.experience };
      const key = [current.jobTitle, current.companyName, current.startDate, current.endDate].join('|').toLowerCase();
      next.experiences = [current, ...list.filter((item) => [item.jobTitle, item.companyName, item.startDate, item.endDate].join('|').toLowerCase() !== key)];
    }
    if (formData.education?.institutionName || formData.education?.degreeMajor) {
      const list = Array.isArray(formData.educations) ? formData.educations : [];
      const current = { ...formData.education };
      const key = [current.institutionName, current.degreeMajor, current.startDate].join('|').toLowerCase();
      next.educations = [current, ...list.filter((item) => [item.institutionName, item.degreeMajor, item.startDate].join('|').toLowerCase() !== key)];
    }
    return next;
  };
  ...
  const handleNext = async () => {
    ...
    const draftToSave = buildCurrentDraft();

    try {
      setFormData(draftToSave);
      await onDraftSave?.(draftToSave);
    } catch (error) {
      showNotice('error', 'Auto Save Gagal', error.message || 'Data belum tersimpan ke backend.');
      return;
    }
  ...
  const goToStep = (index) => {
    ...
    if (index > activeStep + 1) {
      showNotice(
        "error",
        "Selesaikan Step Ini Dulu",
        "Kamu tidak bisa melewati step yang wajib. Klik Next Step dan lengkapi data secara berurutan."
      );
      return;
    }
        ...
        <header className="form-topbar">
          ...
          <div className="topbar-icons">
            ...
            <div className="notification-wrap">
              <button
                type="button"
                className={`topbar-icon-btn notification-btn ${notificationOpen ? "active" : ""
                  }`}
        ...
        <section className="form-content">
          <div className="page-heading">
            <div>
              ...
              <p>
                Lengkapi data kamu step by step untuk membantu proses pembuatan
                CV.
              </p>
            </div>
          </div>
          ...
          <div className="step-progress">
            {steps.map((step, index) => {
              ...
              const isLineDone = index < activeStep;
              ...
              return (
                ...
                >
                  ...
                  {index !== steps.length - 1 && (
                    <span className={`step-line ${isLineDone ? "done" : ""}`} />
                  )}
                </div>
              );
            })}
