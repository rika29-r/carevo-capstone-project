...
import { careerApi, saveFullFormDataToBackend } from "../services/api";
...
function DashboardPage({
  ...
  const [dashboardFormData, setDashboardFormData] = useState(formData || {});
  const [notificationOpen, setNotificationOpen] = useState(false);
  const [notifications, setNotifications] = useState([]);
  const [popupNotice, setPopupNotice] = useState(null);
  const [aiRecommendation, setAiRecommendation] = useState(null);
  const lastAiCategoryRef = useRef('');
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      try {
        const res = await careerApi.recommendation({ updateProfile: true });
        const rec = res?.data || res;
        if (!rec?.recommendedCategory) return;
        setAiRecommendation(rec);
        setDashboardFormData((prev) => {
          const currentTitle = prev?.personalInfo?.professionalTitle;
          if (currentTitle === rec.profileTitle && prev?.personalInfo?.careerInterest === rec.recommendedCategory) return prev;
          const next = {
            ...prev,
            personalInfo: {
              ...(prev.personalInfo || {}),
              careerInterest: rec.recommendedCategory,
              professionalTitle: rec.profileTitle || rec.recommendedCategory,
            },
          };
          queueMicrotask(() => setFormData?.(next));
          return next;
        });
        if (lastAiCategoryRef.current !== rec.recommendedCategory) {
          lastAiCategoryRef.current = rec.recommendedCategory;
          notify('success', 'AI Career Match', `Profil kamu paling cocok ke ${rec.recommendedCategory} (${rec.matchScore}%).`);
        }
      } catch (error) {
        // Jangan ganggu dashboard kalau AI belum bisa dihitung.
      }
    }, 700);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(dashboardFormData?.personalInfo || {}), JSON.stringify(dashboardFormData?.skillList || []), JSON.stringify(dashboardFormData?.experiences || []), JSON.stringify(dashboardFormData?.educations || [])]);
  ...
  const notify = (type, title, message) => {
    ...
    setPopupNotice({ type, title, message });
  };
  ...
  const dashboardRouteMap = {
    ...
    certification: "/dashboard/certifications",
    language: "/dashboard/languages",
    generate: "/dashboard/generate-cv",
  };
  ...
  const goPage = (pageId) => {
    ...
    setNotificationOpen(false);
    const pagePath = dashboardRouteMap[pageId] || "/dashboard";
    if (window.location.pathname !== pagePath) {
      window.history.pushState({ page: "dashboard", activePage: pageId }, "", pagePath);
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const queueBackendSave = (nextData) => {
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);

    saveTimerRef.current = setTimeout(async () => {
      try {
        await saveFullFormDataToBackend(nextData);
      } catch (error) {
        notify(
          "error",
          "Auto Save Gagal",
          error.message || "Data belum tersimpan ke backend. Coba klik Save Changes lagi."
        );
      }
    }, 500);
  };
  ...
  const updateDashboardData = (updater) => {
    setDashboardFormData((prev) => {
      ...
      // Jangan update state App langsung di dalam render child.
      // Ditunda ke microtask supaya tidak muncul warning:
      // "Cannot update a component while rendering a different component".
      queueMicrotask(() => {
        setFormData?.(nextData);
        localStorage.setItem("carevo_form_data", JSON.stringify(nextData));
        localStorage.setItem("carevo_form_completed", "true");
        queueBackendSave(nextData);
      });
  ...
  const sharedProps = {
    ...
    setFormData: updateDashboardData,
    notify,
    goPage,
    aiRecommendation,
  };
  ...
  const renderPage = () => {
    ...
    if (activePage === "profile") return <ProfileInfo {...sharedProps} />;
  ...
  return (
    <div className={`dashboard-shell ${darkMode ? "dark" : "light"}`}>
      <button
        type="button"
        className={`dash-menu-btn ${sidebarOpen ? "hide" : ""}`}
      ...
      {popupNotice && (
        <div className="dash-popup-layer">
          <div className={`dash-popup dash-popup-${popupNotice.type || 'success'}`}>
            <div className="dash-popup-icon">{popupNotice.type === 'error' ? '!' : '✓'}</div>
            <div>
              <h3>{popupNotice.title}</h3>
              <p>{popupNotice.message}</p>
            </div>
            <button type="button" onClick={() => setPopupNotice(null)}>OK</button>
          </div>
        </div>
      )}
      ...
      <aside className={`dash-sidebar ${sidebarOpen ? "show" : ""}`}>
        ...
        <nav className="dash-nav">
          {navItems.map((item, index) => (
            <div key={item.id}>
              {item.group && index > 0 && (
                <p className="dash-nav-group">{item.group}</p>
              )}
        ...
        <div className="dash-sidebar-footer">
          <div className="dash-dark-row">
            ...
            <button
              type="button"
              className={`dash-toggle ${darkMode ? "active" : ""}`}
      ...
      <main className="dash-main">
          ...
          <div className="dash-topbar-icons">
            <div className="dash-notif-wrap">
              <button
                ...
                className={`dash-notif-btn ${notificationOpen ? "active" : ""}`}
                  ...
                  <div className="dash-notif-list">
                    {notifications.length > 0 ? (
                      notifications.map((item) => (
                        <div
                          className={`dash-notif-item notif-${item.type}`}
                          key={item.id}
                        >
                          <span />
                          <div>
        ...
        <footer className="dash-footer">
          <p>© 2026 CAREVO. All rights reserved.</p>
