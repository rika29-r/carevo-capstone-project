...
function findLanguageOption(value) {
...
function getCanonicalLanguageName(value) {
  return findLanguageOption(value)?.name || String(value || "").trim();
}
...
function Language({ formData, setFormData, notify }) {
  ...
  const filteredLanguages = useMemo(() => {
    ...
    // Saat panah diklik, tampilkan semua bahasa.
    // Saat user mengetik, list tetap bisa difilter, tapi input manual tetap diperbolehkan.
    if (showAllLanguageOptions || !keyword) return languageOptions;
  ...
  }, [currentLanguage.language, showAllLanguageOptions]);
  ...
  const addLanguage = () => {
    ...
    const newLanguage = normalizeLanguageItem({
      ...currentLanguage,
      id: Date.now(),
      language: getCanonicalLanguageName(currentLanguage.language),
    });
              ...
              <div className="language-custom-select" ref={languageSelectRef}>
                <div className="language-input-with-flag">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setShowAllLanguageOptions(true);
                  ...
                  <div className="language-custom-menu">
                    {filteredLanguages.length > 0 ? (
                      ...
                      <div className="language-no-result">
                        <strong>Language not found.</strong>
                        <span>Kamu tetap bisa mengetik manual lalu klik Save Language.</span>
                      </div>
                    )}
                  </div>
                )}
